...
**Flux and Flux Pro are scanned separately.** The icons a free Flux component uses land in `flux.free`; icons only a Flux Pro component reaches for land in `flux.pro` (LucideKit skips that group when Pro isn't installed). Flux Pro is a `require-dev` dependency, but its repository needs license auth from a local or global `composer auth.json` that isn't committed — so a contributor without it scans with `flux-pro` absent. When a package isn't in `vendor`, its group is left exactly as it is rather than warned about or pruned: without the source there's no telling "removed upstream" from "not installed here", and `--prune` would otherwise drop working mappings from the shipped config. The practical upshot: run `--prune` only with both packages installed.
